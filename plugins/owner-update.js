...
const handler = async (m, { conn }) => {
    let sentMsg = await m.reply('💢 Actualizando... espera un momento');

    exec('git pull', (err, stdout, stderr) => {
        if (err) {
            conn.sendMessage(m.chat, { text: '... algo salió mal. Déjame intentarlo a la fuerza~ 💢', edit: sentMsg.key }, { quoted: m });
        ...
        if (stdout.includes('Already up to date.')) {
            conn.sendMessage(m.chat, { text: 'Todo ya estaba en orden crack No había nada que actualizar.', edit: sentMsg.key }, { quoted: m });
        } else {
